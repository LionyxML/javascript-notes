...
Source: [[https://medium.com/dhiwise/advanced-javascript-design-patterns-6812f3286585]]

** Creational
Are patterns that will creat objects for you instead of instantiating
and object directly.

*** Factory Method

In this example we have a class of points and we can create a
cartesian point and a polar point.
...
#+RESULTS:
: Point { x: 3.061616997868383e-16, y: 5 }
: Point { x: 5, y: 6 }
: undefined
...
#+RESULTS:
: Person lives at ABC Road, Multan, 123123
: 	   and works at Some Company as a
: 	   Engineer earning 100000000
: 
:  person:  {"city":"Multan","postcode":"123123","streetAddress":"ABC Road","position":"Engineer","companyName":"Some Company","annualIncome":"100000000"}
: undefined

*** Prototype
The prototype pattern is a creational design pattern in software
development.  It is used when the type of objects to creat is
determined by a protitypical instance, which is cloned to produce new
objects.
...
pattern that restricts the instantiation of a classe to one "single"
instance. This is useful when exactly one object ie needed to
coodinate actions across the system.
...
** Structural
This patterns concern class and object composition. They use
inheritance to compose interfaces.
*** Adapter
Is a pattern that allows the interface of an existing class to be used
as another interface. It is used to make existing classes work with
...
class of a set of closely related classes into two separate
...
*** Decorator
The decorator pattern is a design pattern that allows behavior to be
added to an individual object, dynamically, without affecting the
...
architecture, a facade is an object that serves as a front-facing
...
#+RESULTS:
: 10k users take up ~370001 chars
: 10k flyweight users take ~191602 chars
: undefined
...
general form, is a class functioning as an interface to something
else.
...
patterns that identify common communication patterns among objects. By
...
#+RESULTS:
: Dragon (1/1)
: Doubling Dragon's attack
: Increasing Dragon's defense
: Dragon (2/2)
: Doubling Dragon's attack
: Doubling Dragon's attack
: Dragon (8/2)
...
*** Iterator
Iterator accesses the elements of an object without exposing its
...
We will be taking an example of an array in which we print the values
of an array and then by using an iterator we print its value
backwords.
...
*** Mediator
The mediator pattern adds a third-party object to control the
interaction between two objects. It allows loose coupling between
classes by being the only class that has detailed knowledge of their
methods.
...
pattern due to the way it can alter the program’s running behavior. In
object-oriented programming, programs often consist of many classes.

We will be using an example of a person using a chat room. Here, a
chatroom acts as a mediator between two people communicating.
...
#+RESULTS:
: [Alice 's chat session]

*** Memento
Memento restores an object to its previous state.

The memento pattern is a software design pattern that provides the
ability to restore an object to its previous state. The memento
pattern is implemented with three objects: the originator, a caretaker
and a memento.

We will be taking an example of a bank account in which we store our
previous state and will have the functionality of undoing.
...
We will be taking an example of a person in which if a person falls
ill, it will display a notification.
...
We will be taking an example of NumberExpression in which it gives us
...
single algorithm directly, code receives run-time instructions as to
which in a family of algorithms to use.

We will take an example in which we have a text processor that will
display data based on strategy(HTML or Markdown).
...
We will be taking an example of a light switch in which if we turn on
or off the switch, its state changes.
...
*** Template
It defines the skeleton of an algorithm as an abstract class, that how
